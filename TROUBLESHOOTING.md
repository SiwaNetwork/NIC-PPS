...
### 1. PPS сигнал не выходит из разъема INTEL I210

**Симптомы:**
- Кнопки включения/выключения PPS в GUI не работают
- PPS сигнал не появляется на физическом разъеме
- В логах видно использование `/dev/ptp2` вместо `/dev/ptp0`

**Решение (исправлено в v1.1.0):**
- Обновите до версии 1.1.0
- GUI теперь автоматически использует правильное PTP устройство
- Для Intel I210 используется `/dev/ptp0` (SDP пины)

### 2. Ошибка: "PTP устройство не найдено для интерфейса"
...
**Причины:**
- Драйвер igb/igc не загружен
- PTP поддержка отключена в драйвере
- Интерфейс не поддерживает PTP
